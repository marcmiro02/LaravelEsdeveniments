<x-app-layout>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            const idTipusSala = {{ session('id_tipus_sala') ?? 'null' }};
            ...
            let total = 0;
            let selectedEntrades = [];

            if (idTipusSala === 2) {
                // Hardcodeamos la entrada normal para discoteca
                const quantitat = {{ session('quantitatEntrades') ?? 1 }};
                const preuUnitari = 20;
                const subtotal = quantitat * preuUnitari;

                const entrada = {
                    tipus_entrada: "Normal",
                    quantitat: quantitat,
                    preu_unitari: preuUnitari,
                    subtotal: subtotal
                };

                selectedEntrades.push(entrada);

                const listItem = document.createElement('li');
                listItem.textContent = `${entrada.tipus_entrada} - Quantitat: ${entrada.quantitat} - Preu Unitari: ${entrada.preu_unitari}€ - Subtotal: ${entrada.subtotal.toFixed(2)}€`;
                selectedEntradesList.appendChild(listItem);
                total += entrada.subtotal;
            } else {
                // Cargar las entradas de localStorage (caso cine)
                selectedEntrades = JSON.parse(localStorage.getItem('selectedEntrades')) || [];
            ...
            const gestioCost = total * 0.05;
            const ivaCost = total * 0.21;
            const recarrecCost = total * 0.02;
